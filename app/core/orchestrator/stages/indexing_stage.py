...
Uses MilvusClient API with BM25 full-text search support.
"""
import datetime
...
class IndexingStage(PipelineStage):
    ...
    async def execute(self, context: PipelineContext) -> StageResult:
        ...
        try:
            # Get or create BM25-enabled collection using MilvusClient API
            collection_name = milvus_client_manager.get_or_create_collection(
                context.scope_identifier,
                auto_create=True
            )

            self.logger.info(f"📦 Target collection (BM25-enabled): {collection_name}")

            # Prepare Milvus data in dict format (MilvusClient API)
            milvus_data = self._prepare_milvus_data_dict(context)

            # Log data preparation
            self.logger.info(f"📝 Prepared {len(milvus_data)} documents for Milvus (BM25 format):")
            if milvus_data:
                self.logger.info(f"   Fields per document: {list(milvus_data[0].keys())}")
                self.logger.info(f"   ✅ Note: 'id' field auto-generated by Milvus")
                self.logger.info(f"   ✅ Note: 'sparse' field auto-generated by BM25 function")

            # Insert data into Milvus using MilvusClient API
            self.logger.info(f"💾 Inserting {len(milvus_data)} chunks into Milvus (with BM25)...")
            insert_result = milvus_client_manager.insert(
                collection_name=collection_name,
                data=milvus_data
            )

            # Extract inserted IDs from result
            inserted_ids = insert_result.get('ids', []) if isinstance(insert_result, dict) else []

            # Store insert result in context
            context.milvus_insert_result = {
                'insert_count': len(milvus_data),
                'collection_name': collection_name,
                'ids': inserted_ids  # Milvus-generated IDs
            }

            # Log insert statistics
            self._log_insert_stats_dict(context, milvus_data)
            ...
            return StageResult(
                success=True,
                stage_name=self.name,
                message=f"✅ Indexed {len(context.chunks)} chunks to BM25-enabled Milvus collection '{collection_name}'",
                metadata={
                    "insert_count": len(context.chunks),
                    "collection_name": collection_name,
                    "embedding_dimension": len(milvus_data[0]['embedding']) if milvus_data and 'embedding' in milvus_data[0] else 0,
                    "bm25_enabled": True
                }
            )
    ...
    async def rollback(self, context: PipelineContext) -> None:
        ...
        Uses MilvusClient API delete method.
        """
        ...
        try:
            collection_name = context.milvus_insert_result.get('collection_name')
            if not collection_name:
                self.logger.warning(f"No collection name in milvus_insert_result, cannot rollback")
                return

            # Delete by document_id filter (MilvusClient API)
            filter_expr = f'document_id == "{context.document_id}"'
    ...
    def _prepare_milvus_data_dict(self, context: PipelineContext) -> List[Dict]:
        """
        Prepare data in dict format for MilvusClient API with BM25 support

        MilvusClient API expects list of dicts (not column arrays).
        Each dict represents one document/chunk.

        Note:
        - 'id' field NOT included (auto-generated by Milvus)
        ...
        Returns:
            List of dicts, each representing a chunk
        """
        ...
        for i, chunk in enumerate(context.chunks):
            ...
            metadata = {
                ...
                "embedding_size_bytes": len(embedding_list) * 4,  # float32 = 4 bytes
            ...
            # Create document dict (MilvusClient format)
            doc = {
    ...
    def _prepare_milvus_data(self, context: PipelineContext) -> Dict[str, List]:
        """
        LEGACY: Prepare data arrays for old Collection API

        This method is kept for backward compatibility but is not used anymore.
        New code uses _prepare_milvus_data_dict() for MilvusClient API.

        Args:
            context: Pipeline context with all required data

        Returns:
            Dictionary with data arrays matching old Milvus schema
        """
        current_time = datetime.datetime.now()
        document_title = context.filename.replace('.pdf', '')
    ...
    def _log_insert_stats_dict(self, context: PipelineContext, milvus_data: List[Dict]) -> None:
        """
        Log detailed insert statistics for dict-based data (MilvusClient API)

        Args:
            context: Pipeline context
            milvus_data: Prepared Milvus data (list of dicts)
        """
        self.logger.info(f"📊 Milvus Insert Statistics (BM25-enabled):")
        self.logger.info(f"   Collection: {context.get_collection_name()}")
        self.logger.info(f"   Document ID: {context.document_id}")
        self.logger.info(f"   Chunks Inserted: {len(milvus_data)}")

        if milvus_data and 'embedding' in milvus_data[0]:
            emb_dim = len(milvus_data[0]['embedding'])
            self.logger.info(f"   Embedding Dimension: {emb_dim}")

            # Memory usage
            total_embeddings_mb = sum(len(doc['embedding']) * 4 for doc in milvus_data) / (1024 * 1024)
        ...
        self.logger.info(f"   ✅ BM25 Full-Text Search: Enabled")
        self.logger.info(f"   ✅ Sparse Vectors: Auto-generated by BM25 function")
    ...
    def _log_insert_stats(self, context: PipelineContext, milvus_data: Dict) -> None:
        """
        LEGACY: Log detailed insert statistics for array-based data

        This method is kept for backward compatibility but is not used anymore.

        Args:
            context: Pipeline context
            milvus_data: Prepared Milvus data (old format)
        """
